  ...
  try {
    ...
    if (method === "POST" && url === "/v1/verify") {
      ...
      if (Array.isArray(body.tools) && body.tools.length > 0) {
        ...
        let toolCalls: any[] = [];
        if (priorCalls === 0 && toolNames.includes("search_web")) {
      ...
      let content: string;
      if (/Classify this company/i.test(all)) {
...
server.listen(PORT, () => {
  console.log(`Mock API listening on http://127.0.0.1:${PORT} (spider+plunk+openai)`);
});