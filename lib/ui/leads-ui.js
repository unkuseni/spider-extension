...
export function initLeadsUi(ui) {
  ...
  const SETUP_ITEMS = [
    { key: 'spider', label: 'Spider Cloud API key', what: 'powers scraping', ok: (cfg) => !!cfg.spiderApiKey },
    { key: 'ai', label: 'AI key (OpenAI/DeepSeek/…)', what: 'extracts + categorizes contacts', ok: (cfg) => !!cfg.openaiApiKey },
    { key: 'turso', label: 'Turso database', what: 'stores your leads', ok: (cfg) => !!cfg.tursoUrl },
    { key: 'plunk', label: 'Plunk API key', what: 'verifies emails', ok: (cfg) => !!cfg.plunkApiKey },
  ];

  function renderSetupChecklist(cfg) {
    const box = document.getElementById('leadsSetup');
    const items = document.getElementById('leadsSetupItems');
    if (!box || !items) return;
    const rows = SETUP_ITEMS.map((def) => {
      const ok = def.ok(cfg);
      return '<div class="setup-item ' + (ok ? 'ok' : 'missing') + '">' +
        '<span class="setup-check">' + (ok ? '✓' : '✗') + '</span>' +
        '<span class="setup-label">' + escapeHtml(def.label) + '</span>' +
        (ok ? '' : '<span class="muted">— ' + escapeHtml(def.what) + '</span>' +
          '<button class="btn btn-sm btn-outline setup-fix" type="button">Fix</button>') +
        '</div>';
    }).join('');
    items.innerHTML = rows;
    items.querySelectorAll('.setup-fix').forEach((btn) => {
      btn.addEventListener('click', () => chrome.runtime.openOptionsPage());
    });
    const allOk = rows.includes('missing') === false;
    box.classList.toggle('done', allOk);
    box.querySelector('.leads-setup-title').textContent =
      allOk ? '✅ Setup complete — ready to hunt' : '⚙️ Setup — 4 quick steps';
  }
  ...
  async function refreshLeadsConfigState() {
    ...
    renderSetupChecklist(leadsConfig);
  ...
  function renderLeadsSummary(s) {
    ...
    const line = (k, v) => '<div class="leads-summary-line"><span class="k">' + escapeHtml(k) + '</span><span class="v">' + escapeHtml(String(v)) + '</span></div>';
    const rows = [
    ...
    el.innerHTML =
      '<div class="leads-summary-line"><span class="k"><b>' + escapeHtml(s.target) + '</b> <span class="muted">(' + escapeHtml(s.source) + ')</span></span></div>' +
      rows.map(([k, v]) => line(k, v)).join('') +
      (s.errors && s.errors.length
        ? '<div class="leads-summary-meta">⚠️ ' + escapeHtml(s.errors.length) + ' problem(s) — rerun with fewer pages/targets or check your keys</div>'
        : '');
  }
  ...
  function renderLeads(rows) {
    ...
    const html = '<table class="leads-table"><thead><tr>' +
      ...
      '</tbody></table>';
    wrap.innerHTML = html +
      '<p class="field-hint muted" style="margin-top:6px">' +
      'Grades: <b>A</b> Hot → <b>B</b> Warm → <b>C</b> Cool → <b>D</b> Cold · ' +
      '✓ = verified by Plunk · click an email to copy it. ' +
      'Guessed emails (🔮) are pattern-inferred and verified; catch-all domains are detected automatically.</p>';
  ...
  async function doLeadsEnrich() {
    await runLeadsAction(async (db, cfg) => {
      ...
      for (const domain of domains) {
        ...
        if (res.catchAll === true) catchAlls.push(domain);
      }
      ...
      document.getElementById('leadsSummary').innerHTML =
        ...
        (catchAlls.length ? escapeHtml('⚠️ catch-all (guesses skipped): ' + catchAlls.join(', ')) : '') + '\n' +
  ...
  async function doLeadsStats() {
    try {
      ...
      const trustLine = stats.domainsProbed
        ? 'Domains probed for catch-all: ' + (stats.domainsProbed.catch_all ?? 0) + ' catch-all · ' + (stats.domainsProbed.verified_clear ?? 0) + ' clean'
        : '';
      ...
      document.getElementById('leadsSummary').innerHTML =
        ...
        (trustLine ? escapeHtml(trustLine) : '');
    } catch (err) {
  ...
  async function doLeadsExport() {
    try {
      const db = await getLeadsDb();
      const rows = await listLeads(db, { limit: 100000 });
      const cols = ['email', 'email_source', 'person_name', 'title', 'phone', 'company', 'domain',
        'email_type', 'category', 'subcategory', 'tier', 'department', 'seniority', 'decision_maker',
        'lead_score', 'lead_tier', 'icp_match', 'status', 'email_valid', 'interests', 'linkedin',
      ...
      showStatus('Exported ' + rows.length + ' lead(s) to CSV (with scores + segmentation)', 'success');
    } catch (err) {
      showStatus(err.message, 'error');
    }
  }
  ...
  async function runLeadsAction(fn, busyText) {
    ...
    if (missing.length > 0) {
      showStatus('Finish setup first — ' + missing.join(', ') + ' (see the checklist above)', 'error');
      const setup = document.getElementById('leadsSetup');
      if (setup) setup.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    const btns = ['leadsHuntBtn', 'leadsSearchBtn', 'leadsVerifyBtn', 'leadsEnrichBtn', 'leadsScoreBtn', 'leadsEmployeesBtn', 'agentBtn'];
    btns.forEach((id) => { const b = document.getElementById(id); if (b) b.disabled = true; });
    showStatus(busyText + ' (this can take a minute)…', 'info');
    // Live elapsed timer so long runs never feel frozen.
    const timerEl = document.getElementById('statusText');
    const started = Date.now();
    const timer = setInterval(() => {
      if (!timerEl || timerEl.classList.contains('hidden')) return;
      if (!timerEl.textContent.startsWith(busyText)) return; // a later status replaced it
      const secs = Math.round((Date.now() - started) / 1000);
      timerEl.textContent = busyText + ' — ' + secs + 's…';
    }, 1000);
    try {
      const db = await getLeadsDb();
      const cfg = await buildLeadsConfig();
