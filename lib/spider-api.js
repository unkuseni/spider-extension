...
// ---------------------------------------------------------------------------
// Screenshot — POST /screenshot (base64 PNG/JPEG/WebP)
// ---------------------------------------------------------------------------
...
export async function screenshotPage({
  ...
  const body = {
    url,
    cdp_params: { format, full_page: fullPage, ...cdpParams },
    ...(await proxyBody()),
  };
  const response = await fetch(`${SPIDER_API_BASE}/screenshot`, {
    method: 'POST',
    headers,
    body: JSON.stringify(body),
  });
...
// ---------------------------------------------------------------------------
// Transform — POST /transform (HTML → markdown/text)
// ---------------------------------------------------------------------------
...
export async function transformHtml({
  ...
  const response = await fetch(`${SPIDER_API_BASE}/transform`, {
    method: 'POST',
    headers,
    body: JSON.stringify(body),
  });
...
// ---------------------------------------------------------------------------
// Unblocker — POST /unblocker (bot-protection bypass)
// ---------------------------------------------------------------------------
...
export async function unblockPage({
  ...
  const response = await fetch(`${SPIDER_API_BASE}/unblocker`, {
    method: 'POST',
    headers,
    body: JSON.stringify(body),
  });
