...
const PROVIDER_DEFAULTS = {
  openai:    { name: 'OpenAI',     endpoint: 'https://api.openai.com/v1/chat/completions',    model: 'gpt-4o',         key: '' },
...
export async function getAiConfigs() {
  const result = await chrome.storage.sync.get([AI_KEYS_KEY]);
...
export async function setAiConfigs(updates) {
  ...
  await chrome.storage.sync.set({ [AI_KEYS_KEY]: merged });
}
