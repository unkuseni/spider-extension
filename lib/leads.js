...
const LEADS_SETTINGS_DEFAULTS = {
  ...
  crawlLimit: 10,
  guessEmails: false,  // infer employee emails (pattern-based) for people without one
  guessPerPerson: 3,   // max candidate addresses per person
  aiStudio: false,     // use Spider AI Studio /ai/* endpoints (credits apply)
  icpInterests: '',    // comma-separated ICP interest topics (lead scoring)
  icpCategories: '',   // comma-separated ICP categories (lead scoring)
};
...
export async function buildLeadsConfig() {
  ...
  return {
    ...
    crawlDepth: 2,
    spiderProxy: (prefs.spider_prefs || {}).usePremiumProxy === true,
    spiderCountry: (prefs.spider_prefs || {}).proxyCountry || '',
    aiStudio: leads.aiStudio === true,
    ...
    guessEmails: leads.guessEmails === true,
    guessPerPerson: leads.guessPerPerson || 3,
    githubToken: '',
    githubApiBase: 'https://api.github.com',

    icpInterests: (leads.icpInterests || '').split(',').map((s) => s.trim()).filter(Boolean),
    icpCategories: (leads.icpCategories || '').split(',').map((s) => s.trim()).filter(Boolean),
